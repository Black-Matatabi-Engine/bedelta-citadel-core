# `@slivervine/citadel-sdk` — 整合 Blueprint

> **中文參考譯本** · 英文 SSOT：[`CITADEL_SDK_BLUEPRINT.md`](./CITADEL_SDK_BLUEPRINT.md)  
...
**授權：** Apache-2.0 · **實體：** SilverVine Labs  
**套件：** `@slivervine/citadel-sdk`（monorepo：[`src/sdk/`](../../src/sdk/)）  
**EIP-712 domain：** `SliverVineCitadel` · **Version：** `1`  
**主要執行錨點：** **Arbitrum One (`42161`)** — GMX v2 native gateway + Gate domain SSOT  
**Gate verifyingContract：** `SLIVERVINE_GATE_ADDRESS`（[`src/sdk/constants.ts`](../../src/sdk/constants.ts)）

> **Non-inflatable posture：** 本 SDK 在 UserOp / Session Key 簽名**之前**執行 **stateless pre-execution validation**。完整密碼學 quorum 與 replay protection 由鏈上 `SliverVineGate.verifyAndConsume` 強制 — 本套件**不**聲稱完成 off-chain ECDSA recovery。
...
| 支柱 | 範圍 | SDK surface |
|--------|-------|-------------|
| **Pillar 1 — Gatehouse（Account Abstraction & Intent Gateway）** | **Arbitrum One (`42161`)** 為 **primary execution anchor / center of gravity** — GMX v2 GM pools、EIP-712 Gate domain、Agent-Citadel-Guard chainId | `ARBITRUM_ONE_CHAIN_ID`, `SLIVERVINE_GATE_ADDRESS`, `verifyAgentIntent()` |
| **Pillar 2 — Compliance Ingress Firewall（Pillar 2 Reference Escort Adapters）** | Venue-agnostic route policy + capital escort · **`IN_FLIGHT_BRIDGE_CAPITAL`** · **Pending-Capital Recognition Invariant (`lostUsd ≡ 0`)**。**Robinhood Chain / Across (`46630`/`4663` → `42161`) 為 Pillar 2 Reference Escort Adapters** — 非核心錨點。亦治理 Arbitrum-native routes 與 Arb ↔ Hyperliquid hedge channels | `assertUnidirectionalBridge()`, `exportRobinhoodAuditSnapshot()`, `quoteRChainYieldToArbitrumGm()` |
| **Pillar 3 — The Shield** | Sub-ms `checkSoilResistance()` 語義（p50 ~106 μs；Wasm warm &lt;60 μs）保護 **Arbitrum One (GMX v2) native execution**、**Arb ↔ Hyperliquid cross-venue routing**、**Arbitrum Edge Worker AI Agents** | `verifyAgentIntent()`, `evaluateSoilCore()`, `guardAgentUserOp()`, legacy-risk re-exports |
...
封閉式三場路由，**Arbitrum One 為 primary yield base**。Hyperliquid 提供 Δ-neutral hedge leg；Robinhood Chain 為 **optional permissioned ingress** — 永不作 secondary execution anchor。
...
**Control plane：** Edge Worker（`SystemState` SSOT）在任何 unsigned GMX payload 或 HL hedge dispatch 之前評估 sequencer · oracle lag · soil · RPC radar。**Read API：** `GET /api/yield/triangle`。

---

## Core Architectural Invariants

這些不變量在 SDK、Edge Worker、鏈上 Gate 之間 **不可妥協**。整合者必將違規視為硬失敗 — 永不作可重試軟錯誤。

### 1. Non-Custodial Unidirectional Escort — Pending-Capital Recognition Invariant (`lostUsd ≡ 0`)

| 屬性 | 規則 |
|----------|------|
| **Capital custody** | 協議永不將用戶本金記為協議自有；資本保留於用戶 Kernel account 或 venue GM / HL position |
| **Bridge direction** | Robinhood `46630`/`4663` → Arbitrum `42161` outbound only；reverse path ⇒ `AML_INBOUND_TO_ROBINHOOD_BLOCKED` |
| **In-flight accounting** | Pending bridge liquidity 標記 `IN_FLIGHT_BRIDGE_CAPITAL`；**Pending-Capital Recognition Invariant：** **`lostUsd` 恆為 `0`** — 協議在 active execution 期間永不提前將 in-flight bridge capital 核銷為損失（直至 explicit timeout `BRIDGE_TIMEOUT_FAIL_CLOSED`） |
| **SDK enforcement** | `assertUnidirectionalBridge()` · `exportRobinhoodAuditSnapshot()` · `buildRobinhoodAuditSnapshot()` 在 `lostUsd ≠ 0` 時 throw |

Inbound reverse blocking **hard-coded** — 零 inbound capital 至 Robinhood 為不變量，非 runtime toggle。

### 2. 30s TTL Heartbeat / Intent Execution Window — Nonce-Healed Self-Exploding Session Keys

Ephemeral session keys 設計為在 TTL breach、heartbeat loss 或 nonce desync 時 **self-destruct** — 關閉簽章通道而非以 stale authority 重試。**30s TTL Heartbeat / Intent Execution Window** 與底層密碼學 session key lifetime（模組範圍內最高 **24h / 7d**）不同。
...
Session keys 為 **scoped clips** — SDK default clip **≤ $30**（`agent-intent.ts`）；protocol hard ceiling **R07 $5,000**（`SESSION_KEY_NOTIONAL_CAP_USD` in `session-key-gates.ts`）— 非 hot-wallet 替代品。

### 3. Zero-Bundler-Rejection Invariant（EIP-7562 Compliance）

Citadel UserOps 在 validation phase **不得** 觸發 bundler rejection（[EIP-7562](https://eips.ethereum.org/EIPS/eip-7562) opcode/storage rules）。Bundler failure 為 **protocol fault**，非 blind-retry signal。
...
另見：[`02_STANDARD_COMPLIANCE_AND_EIP_WIKI.md`](../architecture/02_STANDARD_COMPLIANCE_AND_EIP_WIKI.md) — EIP-7562 wiki entry。

### 4. CaaS Monetization — 10 bps GMX Native Builder Fee + Up to 25% GMX Referral Rebate

Citadel 將 institutional flow 路由至 GMX GM pools **underweight side**，捕捉 builder revenue、GMX referral rebates 與 positive skew rebates，同時在 Hyperliquid 維持 Δ-neutral hedge。

> **Execution safety：** **10 bps** builder fee 由 GMX v2 **ExchangeRouter** `uiFeeReceiver` parameters（`GMX_UI_FEE_BPS`）原生支持。對 v1.0 sub-ms fail-closed execution path **零額外 overhead** — SDK/Worker 僅在未簽名 payload 注入 fee field；soil fuse、session gates、Gate attestation 不變。
...
SDK 在 Worker 注入 `uiFeeReceiver` **之前** gates signing；skew routing decisions 在 Edge soil + balancer — 非 SDK exports。
...
Robinhood Chain 為 Pillar 2 的 **inaugural Code-Verified / Dry-Run Verified reference adapter** — venue-agnostic unidirectional AML escort 與 Pending-Capital Recognition Invariant（`IN_FLIGHT_BRIDGE_CAPITAL`, `lostUsd ≡ 0`）。**非** protocol anchor。
...
`checkSoilResistance()`（Edge Worker）與 SDK `evaluateSoilCore()` / `verifyAgentIntent()` 共享相同 soil fuse 語義：
...
SDK gates signing on **Arbitrum One (`42161`)**。GMX unsigned payload construction（含 10 bps `uiFeeReceiver`）為 Worker-side：
...
**Dune telemetry:** [Live dashboard](https://dune.com/silvervinelabs/silvervine-citadel-telemetry) · [`DUNE_DASHBOARD_SPECIFICATION_ZH.md`](../telemetry/DUNE_DASHBOARD_SPECIFICATION_ZH.md)（production DuneSQL feed + activity chart + reconciliation panels）。
...
pnpm test -- --run # 836 tests
pnpm audit:fast
pnpm build:wasm # rebuild pkg/soil_core.wasm
```

**Vitest SSOT:** `836 tests`
