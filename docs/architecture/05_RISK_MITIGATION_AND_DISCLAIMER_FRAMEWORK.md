...
> **Baseline:** Vitest **222 test files | 1044 PASS clean (100%)** · Wasm hot-path **50.94 KiB gzip** (`pkg/soil_core.wasm` **< 28 KiB**) · Shield **p50 ~106 µs**
...
| Anchor | Value |
|--------|-------|
| **Vitest baseline** | **222 test files | 1044 PASS clean (100%)** · `pnpm test -- --run` |
...
│ · Vitest SSOT: 222 test files | 1044 PASS clean
└────────────────────────────────────────┬────────────────────────────────────────┘
                                         │
                                         ▼
┌─────────────────────────────────────────────────────────────────────────────────┐
...
|-------|--------|-------------------|
| **A — V1.0** | ✅ Code-Verified (**222 test files | 1044 PASS clean (100%)**) | Arbitrum One GMX v2 / HL Δ-neutral · Wasm Shield · [ERC-8196](https://eips.ethereum.org/EIPS/eip-8196) (Final) policy gate · Gate `0xb174118b…` · Sepolia + One consume-once Gate |
...
| **Regression bar** | **222 test files | 1044 PASS clean (100%)** | No Wasm rewrite · additive swarm tests | No Wasm rewrite · CaaS SDK contract tests |
...
**Code anchors:** `src/services/yield/rebalance-rules.ts` · `src/services/adapters/gmx-v2-order-payload.ts` · `src/services/risk-control-lib/soil-resistance.ts` · Vitest **222 test files | 1044 PASS clean
...
| **Model validation** | Vitest **222 test files | 1044 PASS clean (100%)** full regression | CI / pre-release |
...
|-------|-------------------|----------|
| Full regression | `pnpm test -- --run` | **222 test files | 1044 PASS clean (100%)** |
