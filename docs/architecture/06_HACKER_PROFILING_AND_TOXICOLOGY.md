# Hacker Profiling & Cyber-Zen Toxicology Framework

> **Product:** **SliverVine ExoMesh** (Module A) · **SliverVine Sanctuary** (Module B)  
> **Frameworks:** FBI **Mindhunter** behavioral profiling (MO · signature · escalation) · **Tang Sect** toxic-medical duality (poison as cure · hidden blade)  
> **Defense SSOT:** R01–R20 Defense Matrix · `checkSoilResistance()` · EIP-1193 Retail Guard · **Vitest:** **228 test files | 1064 PASS clean**  
> **Architecture index:** [`README.md`](./README.md) · [`03_DEFENSE_MATRIX_AND_WASM_CORE.md`](./03_DEFENSE_MATRIX_AND_WASM_CORE.md) · [`04_STANDARD_COMPLIANCE_AND_EIP_WIKI.md`](./04_STANDARD_COMPLIANCE_AND_EIP_WIKI.md) · [`../citadel/ERC7540_ASYNC_ESCORT_BREAKTHROUGH.md`](../citadel/ERC7540_ASYNC_ESCORT_BREAKTHROUGH.md)

---

## Executive Thesis — Toxic-Medical Duality & Hidden Blades

In Tang Sect lore, the deadliest arts are **invisible**: a poison administered as medicine, a blade drawn before the opponent knows they have been cut. SliverVine Protocol maps this duality onto Web3 defense:

| Tang Sect principle | SliverVine engineering analog |
|---------------------|------------------------------|
| **毒 (poison)** | Toxic calldata · MEV probes · scraper traffic · async vault drift |
| **医 (medicine)** | Same signals **tighten** `checkSoilResistance()` thresholds · honeypot decoys **inoculate** forked frontends |
| **暗器 (hidden blade)** | **Sub-1.8µs** Wasm soil reflex · **p50 ~15µs** `rootProtection()` severance — attacker payload never reaches mempool |
| **点穴 (pressure point)** | `INTENT_RING_U32` attempt budget · **4th-strike** physical hot-key severance |

**Mindhunter** adds the investigative layer: every on-chain adversary exhibits a **modus operandi (MO)** — burst retries, batch concealment, permission drift. ExoMesh does not wait for post-mortem analytics; it **profiles signatures in-process** and severs at the pressure point.

```text
                    MINDHUNTER (profile)              TANG SECT (counter)
                    ─────────────────              ───────────────────
Signature A Burst   → INTENT_RING_U32              → 4th-strike severance
Signature B Batch   → EIP-5792 calls[] unfold      → hidden calldata blade
Signature C Drift   → ERC-7540 setOperator lock      → Sanctuary zero-trust
Scraper / fork MO   → C1 honeypot 99% slippage      → decoy poisoning
MEV probe MO        → jittered soil thresholds       → toxic ingress vaccine
```
...
| **Chaos tier** | **C2** — `retail-guard-provider.test.ts` **35/35** |
...
| Layer | Mechanism |
|-------|-----------|
| **Calldata blade** | `calldata-parser.ts` · `calldata-hex.ts` — u32 selector dispatch on `CALLDATA_SCRATCH` |
| **Wasm FFI** | `wasm-adapter.ts` — `DataView` on Wasm linear memory · `BIGINT_U32_LUT` (zero per-gate `BigInt` alloc) |
...
| Async drift | `\|claim − request\| × 10⁴ / request > maxBps` → `ERC7540_ASYNC_SLIPPAGE_DRIFT` |
...
**Chaos tier:** **C3** — `erc7540-async-escort.test.ts` **3/3** `[Sanctuary]`

**SSOT:** [`erc7540-async-escort.ts`](../../src/sdk/eip1193-agentic-wallet-guard/erc7540-async-escort.ts) · [`ERC7540_ASYNC_ESCORT_BREAKTHROUGH.md`](../citadel/ERC7540_ASYNC_ESCORT_BREAKTHROUGH.md)
...
## Part II — Tang Sect Toxicology (Honeypot Trap Hosts)

### Cyber-Zen Principle — 以毒攻毒 (Fight Poison with Poison)

The Tang Sect does not merely **block** toxins — it **studies** them to refine antidotes. SliverVine applies the same inversion to adversarial traffic:

1. **Feed decoys** to unauthenticated scrapers (poison for the attacker)
2. **Harvest probe telemetry** to tighten legitimate-user thresholds (medicine for the protocol)
...
### Decoy Poisoning — Chaos Level C1

**Tang Sect analog:** 断肠散 (Gut-Breaking Powder) administered to enemy scouts — they report false intelligence to their commanders.
...
| **Gate eval** | `evaluateRpcDefenseGate()` · `isRpcDefenseAuthenticated()` ([`rpc-fetch-gate-eval.ts`](../../src/services/defense/rpc-fetch-gate-eval.ts)) |
...
**Verification:** `npx vitest run tests/defense/rpc-whitelist.test.ts` · retail guard honeypot swap/depth trips in `retail-guard-provider.test.ts`

---

### Toxic Ingress as Vaccine — Dynamic `checkSoilResistance()` Sensitivity

**Tang Sect analog:** 以毒养元 — exposing the body to controlled toxins builds immunity. SliverVine exposes the **soil fuse** to controlled adversarial probes so production thresholds **harden** rather than static-decay.
...
### Chaos Level C1–C3 — Toxicology Test Grid

| Level | Tang Sect metaphor | Target MO | Verification |
|-------|-------------------|-----------|--------------|
| **C1** | Decoy poison for scouts | Unauthenticated RPC scrapers | `tests/defense/rpc-whitelist.test.ts` `[ExoMesh]` |
| **C2** | Hidden blade vs batch concealment | Burst retry + `wallet_sendCalls` masking | `retail-guard-provider.test.ts` **35/35** · `eip5792-send-calls.test.ts` **3/3** |
| **C3** | Pressure point on async vault | Oracle lag · de-peg · ERC-7540 drift | `pnpm demo:usdai -- --trip` · `erc7540-async-escort.test.ts` **3/3** `[Sanctuary]` |

> **Honest boundary:** C1–C3 are **in-process Mini-Chaos** proofs. Large-scale K8s / Sequencer outage chaos is **NOT RUN** — disclosed per [`0911_offwork_Grok_zh.md`](../opsec/0911_offwork_Grok_zh.md).

---

## Part III — Sub-1.8µs Hidden Blade (ExoMesh Reflex Arc)

### The Blade Drawn Before the Mempool Knows

**Tang Sect analog:** 无形剑 (Formless Sword) — the cut lands before the victim perceives the draw.
...
T3  ExoMesh pre-consensus reflex ← HIDDEN BLADE
    ├─ Pure invariant math     ~0.5–1.1 µs
    ├─ Wasm soil_core warm     < 1.8 µs (demo wasmUs lane)
    ├─ Wasm reflex severance   p50 ~15 µs (FAIL_CLOSED)
    └─ E2E Edge Shield         p50 ~106 µs (ALLOW path)
```
...
| **3. Evaluate** | `checkSoilResistance()` + `evaluateRetailRisk()` | Wasm bitmask parallel eval |
...
|-----------------|---------|----------------------|----------------------|--------|
| **Mindhunter** | Signature A — Burst | `intent-core-ring.ts` | `MAX_ATTEMPTS_EXCEEDED_SEVERED` | `intent-sinking-audit.test.ts` **11/11** |
| **Mindhunter** | Signature B — Batch | `eip5792-send-calls.ts` | `SEND_CALLS_BATCH_REJECTED` · per-call codes | `eip5792-send-calls.test.ts` **3/3** |
| **Mindhunter** | Signature C — Drift | `erc7540-async-escort.ts` | `ERC7540_OPERATOR_REJECTED` | `erc7540-async-escort.test.ts` **3/3** |
| **Tang Sect** | Decoy poisoning C1 | `rpc-fetch-gate-eval.ts` | `HONEYPOT_ACTIVE` · 99% slippage | `rpc-whitelist.test.ts` |
| **Tang Sect** | Toxic vaccine | `soil-resistance-jitter.ts` | Jittered `slippageFuse` · mask commit | `protocol-mask-sync.test.ts` |
| **Hidden Blade** | Sub-1.8µs reflex | `pkg/soil_core.wasm` | `SOIL_TRIPPED` · `rootProtection()` | `pnpm demo:gmx -- --trip` |
...
# Tang Sect C1 — honeypot trap hosts
npx vitest run tests/defense/rpc-whitelist.test.ts

# Hidden Blade — live FAIL_CLOSED severance
pnpm demo:gmx -- --trip
...
| Document | Purpose |
|----------|---------|
| [`03_DEFENSE_MATRIX_AND_WASM_CORE.md`](./03_DEFENSE_MATRIX_AND_WASM_CORE.md) | R01–R20 matrix · Zero-GC ring slab · Wasm moats |
| [`04_STANDARD_COMPLIANCE_AND_EIP_WIKI.md`](./04_STANDARD_COMPLIANCE_AND_EIP_WIKI.md) | ExoMesh competitive matrix · EIP compliance |
| [`../citadel/ERC7540_ASYNC_ESCORT_BREAKTHROUGH.md`](../citadel/ERC7540_ASYNC_ESCORT_BREAKTHROUGH.md) | Sanctuary ERC-7540 deep dive |
| [`02_THREE_PILLARS_AND_INGRESS_PIPELINE.md`](./02_THREE_PILLARS_AND_INGRESS_PIPELINE.md) | Honeypot RPC · legacy vs engineered standards |
...
**Framework one-liner:** Mindhunter profiles the attacker's **MO**; Tang Sect **poisons the poison** with honeypot decoys and threshold vaccines; ExoMesh draws the **hidden blade** at sub-1.8µs — severing access before the mempool ever sees the payload.
