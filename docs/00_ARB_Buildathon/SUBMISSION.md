...
| **Third-party bundlers** | **DISCLOSED OUT OF SCOPE** if unintegrated | Must wire `withExoMeshShield` / `verifyAgentIntent` upstream of relayer |
...
| **Vitest baseline** | **244 test files | 1126 PASS** · `pnpm test -- --run` · `pnpm exec tsc --noEmit` **tsc 0 errors** · ring-slab heap gate **&lt;16 KiB** ([`intent-sinking-audit.test.ts`](../../tests/core/intent-sinking-audit.test.ts)) |
...
| **Production baseline** | Phase A+B+C mainnet verified · **244 test files | 1126 PASS** · **Cargo 2/2** · **58.72 KiB gzip** Worker bundle |
...
1. **Native LLM Back-off & Retry Intercepts**: Active **60-second cooldown lock** per `agentId` in `withExoMeshShield` ([`src/sdk/decorator.ts`](../../src/sdk/decorator.ts)) prevents token-burning infinite retry loops and **RPC Rate-Limit Self-DoS** when transactions fail closed — `[Citadel Back-off] MANDATORY_COOLDOWN_ACTIVE` surfaced via Retail Guard SDK and B2B decorator (`pnpm demo:agent -- --trip`).
