...
> **中文參考譯本** · 英文 SSOT：[`DUNE_DASHBOARD_SPECIFICATION.md`](./DUNE_DASHBOARD_SPECIFICATION.md)  
...
**實體：** SilverVine Labs · **Live SSOT：** `GET /api/grant-audit`  
**受眾：** Buildathon evaluators · Dune sponsor diligence · institutional allocators  
**對账：** On-chain `SliverVineGate` events + grant-audit `duneTelemetry` KV snapshots。

**狀態：** Live Log-Engine Verified · **Public Dashboard Published**
...
| **PEV metric** | **Prevented Exploit Volume (PEV)** — `SUM(blocked_intent_notional_usd)` from `RiskTripBlocked` logs（Sepolia Gate 上 fully operational） |
| **Off-chain anchor** | `/api/grant-audit` → `duneTelemetry.responseRef`（sha256） |

> **澄清：** Live Dune dashboard at [silvervine-citadel-telemetry](https://dune.com/silvervinelabs/silvervine-citadel-telemetry) 顯示來自 Sepolia Gate `0xb174118bC0B84e8D6D59EEF2339e29bF7FCf8BF1` 的 **兩條並行 on-chain streams**：
> 1. **`IntentAttested`** — 即時 EIP-712 intent attestations（PASS / emergency de-leverage greenlights）。
> 2. **`RiskTripBlocked`** — pre-broadcast fail-closed severance events（toxic intent blocked at 0-Gas；feeds **PEV**）。
>
> Dune engine **actively ingests decoded events** from Sepolia Gate and reconciles against live `duneTelemetry` snapshots from the Edge Worker。
