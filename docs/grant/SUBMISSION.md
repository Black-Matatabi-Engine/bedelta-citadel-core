...
**Hero:** BeDelta Living Water — Hyperliquid-native yield ingress + 2PC + Session Key  
...
|----------|---------------------------|
| Read-path ingress · testnet 2PC proof | Live capital bridge |
...
**Also:** `pnpm test` → **575/575** · `pnpm grant:hl-testnet` → 2PC audit JSON
...
```bash
curl -s "https://<your-worker>/api/yield/triangle?symbol=ETH" \
...
```bash
curl -s "https://<your-worker>/api/telemetry/health" \
...
Structured JSON: CREATE → PREPARE → COMMIT (and PREPARED-only ABORT demo).

---

## Ask Framing (suggested)

**Use of funds:** harden Wave 1 HL testnet path · Milestone 2 capital bridge · DO ledger.  
