...
| **Verified commit** | `main` @ **`3f26efa`** (Citadel-Armor SSOT) · baseline **`572e5cd`** (Phase A+B+C mainnet) · Worker bundle **163.72 KiB raw | 57.81 KiB gzip** (`limitKiB: 150` · `pass: true`) | `git rev-parse HEAD` · `pnpm bundle:measure` |
...
| **Dual-Engine Wasm (FROZEN)** | **Engine A** Stylus `0xc23587…` · **Engine B** Edge SHA-256 `67f8fcc7…` — see [§ Dual-Engine Map](#dual-engine-infrastructure-map-frozen--2026-09-10) | [`01_ON_CHAIN_MAINNET_ANCHORS.md`](./03_ON_CHAIN_MAINNET_ANCHORS.md#dual-engine-infrastructure-map-frozen--2026-09-10) |
...
| **B2B agent decorator** | `withExoMeshShield` (legacy: `withCitadelShield`) · `verifyAgentIntent()` — framework-agnostic pre-broadcast wrapper | [`decorator.ts`](../../src/sdk/decorator.ts) · `pnpm demo:agent` |
...
| **Agent SDK decorator** | `withExoMeshShield` (legacy: `withCitadelShield`) — zero-touch pre-broadcast wrapper | [`src/sdk/decorator.ts`](../../src/sdk/decorator.ts) · [`examples/agent-interceptor-demo.ts`](../../examples/agent-interceptor-demo.ts) |
