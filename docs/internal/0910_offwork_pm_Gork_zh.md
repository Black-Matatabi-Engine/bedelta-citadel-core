...
> 評分機制：**SC**（安全與正確性）· **PMF**（產品市場契合）· **Inno**（創新）· **RPS**（可重現性與證明面）。**總分** = 四維算術平均。英文工程 SSOT：`src/services/api/pendle-shield/` · `docs/sdk/05_PENDLE_YIELD_SHIELD_API_SPEC.md` · `src/guards/pendle-gmx-cross-guard.ts`。
...
| 公開規格 | `docs/sdk/05_PENDLE_YIELD_SHIELD_API_SPEC.md` |
