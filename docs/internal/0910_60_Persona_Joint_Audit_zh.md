...
| **Pillar Set X — E2E Harness** | `scripts/grant-delta-neutral-demo.ts` | ✅ `pnpm demo:delta-neutral` 4/4 · `pnpm demo:delta-neutral -- --unwind` 5/5 Step R20 回收 |
...
| **Pillar Set X vs Y 文件** | ✅ | `docs/05_pitch_and_demos/02_CLI_DEMO_RUNBOOK.md` · `--unwind` 資本回收 vs pre-consensus `--trip` |
...
- **Vitest：** `tests/core/intent-drift.test.ts` — GMX 批准 vs Pendle 目標
...
| 0:32 | *Pillar Set X：Sovereign Vault — `pnpm demo:delta-neutral` 四步 · 可選 `--unwind` Step 5 資本回收。* |
