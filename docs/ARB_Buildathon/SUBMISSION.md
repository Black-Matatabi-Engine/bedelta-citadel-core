...
| **[EIP-8079](https://eips.ethereum.org/EIPS/eip-8079) / [EIP-8105](https://eips.ethereum.org/EIPS/eip-8105)** — Pre-Consensus 0-Gas Gateway | Transactions enter mempools blind — users exposed to L2 Sequencer reordering/MEV without a 0-Gas withdrawal mechanism | SilverVine acts as a **Client-Side Preconf Gateway** — [`soil-resistance-core.ts`](../../src/core/soil-resistance-core.ts) + `pkg/soil_core.wasm` simulate preconfirmations in Wasm · [`guard-engine.ts`](../../src/sdk/exomesh-agentic-wallet-guard/guard-engine.ts) aborts unsafe transactions locally before network broadcast · **p50 ~15µs** reflex severance on `--trip` | `npx vitest run tests/core/protocol-mask-sync.test.ts` **6/6 PASS** · `pnpm demo:gmx -- --trip` |
