...
| **Verified Commit** | `main` @ **`abd8518`** — Phase A+B+C **mainnet deploy** · dual-wallet workflow SSOT · **216/964** Vitest · **Cargo 2/2** · **50.94 KiB gzip** |

### Core Sinking SSOT (`src/core/`)

| Module | Scope |
|--------|--------|
| [`gmx-risk-core.ts`](../../src/core/gmx-risk-core.ts) | GMX GM wire audit SSOT · mirrors Phase A `GmxRiskInvariantLib` (**131 LOC**) |
| [`risk-engine-core.ts`](../../src/core/risk-engine-core.ts) | Risk bitmask core · split under Q1 <180 LOC (**73 LOC**) |
| [`soil-resistance-core.ts`](../../src/core/soil-resistance-core.ts) | Soil math · time gates · jitter (**60 LOC**) |
| [`risk-engine-usdai.ts`](../../src/core/risk-engine-usdai.ts) | USD.ai invariants · pure clock SSOT |
| [`session-key-guard-core.ts`](../../src/core/session-key-guard-core.ts) | Session key validity · notional math |
| [`delta-neutral-calculator.ts`](../../src/core/delta-neutral-calculator.ts) | 0-Δ hedge sizing |
| [`funding-regime-core.ts`](../../src/core/funding-regime-core.ts) | Funding regime math · leverage scaling |

Legacy paths in `src/adapters/` and `src/services/` maintain **100% backward compatibility** via thin-shell re-exports. **Solidity ingress:** `SliverVineRiskOracle.sol` · `IngressSafetySwitch.sol` upgraded to **Custom Errors** (`revert CustomError()`) while preserving `ERR_*` bytes32 telemetry events.
...
| **Stylus 96KB expansion readiness** | **`SliverVineSoilCoprocessor`** (mainnet `0xc23587d6…`) + **Phase C `citadel_invariants`** Stylus coprocessor · Stylus SDK **0.10.7** · `cargo test stylus_core` **5/5** · `citadel_invariants` Cargo **2/2** · TS/Rust parity **6/6** · `pnpm build:citadel-invariants` | `pkg/soil_core.wasm` · `pkg/citadel_invariants.wasm` · [`contracts/citadel_invariants/`](../../contracts/citadel_invariants/) |

---

## Production Architecture Declarations

> Authoritative v1.0 production scope for SliverVine Citadel Shield. **Baseline:** `main` @ **`abd8518`** — **216 test files | 964 PASS\|216 test files | 964 PASS Clean (100% PASS)** · `tsc --noEmit` **0 errors** · Cargo `citadel_invariants` **2/2**.

| # | Domain | Production declaration |
|---|--------|------------------------|
| **1** | Ephemeral Ignition Signers | `0x1111…`/`0x2222…` are **Ephemeral Verification Signers** on Mainnet Gate `0xb174…` — **public verification + sandbox reproducibility only**; production HSM custody excluded by design. Gate shape = consume-once EIP-712; **post-launch governance rotation** via native `proposeAdmin` / `acceptAdmin`. → [On-Chain Verification — Arbitrum One](#on-chain-verification--arbitrum-one-42161) |
| **2** | GMX v2 Pre-Flight Guards | **Phase A+B+C invariant stack** (`GmxRiskInvariantLib` · `GmxSoilMatrixSwitch` single-SLOAD · `citadel_invariants` Stylus coprocessor + PolicyGuardV2 Solidity fallback). GMX v2 execution guards verified via **Vitest CLI + dry-run pipelines** — **0-Gas pre-flight** severance. **GM Pool I/O channel CLOSED** on `42161`: deposit [`0xe3155220…`](https://arbiscan.io/tx/0xe3155220e464c375329838bb5ca8498226b8c8fa32c11929b7605070f7be4774) · approve [`0x30ec0b7a…`](https://arbiscan.io/tx/0x30ec0b7a9493f0c43edb257fd40f6d6f9258401e206357f3db7574b11071b00e) · withdraw [`0xfd3601dc…`](https://arbiscan.io/tx/0xfd3601dce5c2407d371186d8a24829994547ec8810f4a20c3e798d2fb67ae410). → [§4 GMX](#4-gmx) |
...
| **4** | Telemetry Infrastructure | **Sepolia (`421614`): Active Live Event Stream** · **Arbitrum One (`42161`): Contracts Anchored + SQL Query Specs Ready for Ingest** — zero misleading cross-chain claims; partitions documented separately. → [§5 Dune](#5-dune-analytics) |
| **5** | Agent Integration | **V1.0 Live Native Integrations** — Wayfinder · ElizaOS · Virtuals · LangChain · Stabilizer · `pnpm demo:{wayfinder,elizaos,virtuals,langchain,stabilizer,quad}` · [`src/adapters/`](../../src/adapters/) · **216 test files | 964 PASS\|216 test files | 964 PASS Clean (100% PASS)** → [Four Major AI Agent Frameworks](#four-major-ai-agent-frameworks-v10-live--full-quad-coverage) |
...
| **7** | Dune Analytics | **Sepolia: Active Live Event Stream** · **Arbitrum One: Contracts Anchored + SQL Specs Ready for Ingest** (not live mainnet stream). → [§5 Dune](#5-dune-analytics) |
| **8** | Commercial Model | **v1.0 = Public Open Gateway** (`X-Citadel-Tier: public`) · **V1.1 = 4-Tier SaaS** (Milestone 1) · **V2.0 = Institutional CaaS & Orbit Shield**. → [Business Model](#business-model--gtm-strategy) |
...
| **USD.ai** | Arbitrum One | sUSDai peg drift > **30 bps** · oracle lag · depth < **$100k** | [`risk-engine-usdai.ts`](../../src/core/risk-engine-usdai.ts) (SSOT) · [`usdai-adapter.ts`](../../src/adapters/usdai/usdai-adapter.ts) · **AI-Compute Yield Collateral — Guarded by Citadel Soil Fuse** |
...
- **Tests:** [`tests/adapters/stabilizer-adapter.test.ts`](../../tests/adapters/stabilizer-adapter.test.ts) · [`tests/adapters/uniswap-v3-adapter.test.ts`](../../tests/adapters/uniswap-v3-adapter.test.ts) · **216 test files | 964 PASS Clean (100% PASS)**
...
- **Tests:** [`wayfinder-shield.test.ts`](../../tests/adapters/wayfinder-shield.test.ts) · [`elizaos-plugin.test.ts`](../../tests/adapters/elizaos-plugin.test.ts) · [`virtuals-adapter.test.ts`](../../tests/adapters/virtuals-adapter.test.ts) · [`langchain-tool.test.ts`](../../tests/adapters/langchain-tool.test.ts) · **216 test files | 964 PASS Clean (100% PASS)**
...
|---------------|---------------|
| Three Pillars · R01–R20 | [Architecture index §01–03](../architecture/README.md) · [Three Pillars](../architecture/02_THREE_PILLARS_AND_INGRESS_PIPELINE.md) · [Defense Matrix](../architecture/03_DEFENSE_MATRIX_AND_WASM_CORE.md) |
...
* **Test Suite**: **216 test files | 964 PASS Clean (100% PASS)** — re-run `pnpm test -- --run` to confirm. Full matrix: [Verification Matrix](../VERIFICATION_MATRIX.md).
...
| **On-Chain Settlement** | PolicyGuardV2 `0xfd98cadb…` · MatrixSwitch `0x4129aee9…` · RiskOracleV2 `0xfadb1475…` | Phase A+B+C **Verified Live** @ `abd8518` · **`stylusCoprocessor=0`** → Pure Solidity fallback · 100% fail-closed without separate Stylus mainnet activation |
...
|---------|--------|-------|
| **V1.0** | ✅ Code-Verified Live Baseline | Arbitrum One GMX v2 ETH/USDC GM + HL 1× short · Wasm `checkSoilResistance()` p50 ~106µs · **V1.0 Live Native Agent Integrations** — Wayfinder · ElizaOS · Virtuals · LangChain · Stabilizer ([`src/adapters/`](../../src/adapters/)) · [ERC-8196](https://eips.ethereum.org/EIPS/eip-8196) (Final) policy pre-validation · EIP-712 consume-once Gate `0xb174118bC0B84e8D6D59EEF2339e29bF7FCf8BF1` · Dune + SHA-256 dual-source `GET /api/grant-audit` · **public open gateway** (`X-Citadel-Tier: public` · 5 RPS) · Worker bundle **50.94 KiB gzip** · **216 test files | 964 PASS\|216 test files | 964 PASS Clean (100% PASS)** |
...
| **Pendle** | **Institutional Sentinel + AI Guarded Pool Factory** (V1.0 Core Pillar 3) | 60s TTL Oracle Fuse · 200bps Jitter Guard · `validateAIPoolSelection()` 5 Invariants · protocol-tax-free · SaaS credits in V1.1 | `src/adapters/pendle/pendle-market-oracle-adapter.ts` · `src/adapters/pendle/pendle-pool-factory-adapter.ts` · `src/guards/pendle-gmx-cross-guard.ts` |
...
* **Phase A+B+C on-chain invariant stack (@ `abd8518`):**
...
| **M-CLI** | Vitest **216 test files | 964 PASS\|216 test files | 964 PASS Clean (100% PASS)** | All | ✅ Delivered |
...
**Regression bar:** Vitest **216 test files | 964 PASS Clean (100% PASS)** · **3-Tier Security Matrix: 5/0/0 PASS (Vitest, Forge, Slither, Aderyn, pnpm-audit)** · Forge 60/60 · Cargo Stylus 9/9 · Worker bundle **50.94 KiB gzip** (`pnpm bundle:measure` · pass &lt;75 KiB) · Wasm **<28kb Cloudflare budget, <60µs execution** · Shield **p50 ~106µs**.
