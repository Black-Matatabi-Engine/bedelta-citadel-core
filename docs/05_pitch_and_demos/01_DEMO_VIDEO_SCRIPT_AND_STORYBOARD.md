...
| **Related SSOT** | [`VERIFICATION_MATRIX.md`](../06_verifications/01_VERIFICATION_MATRIX.md) · [`README.md`](../01_architecture/README.md) · [`05_RISK_MITIGATION`](../01_architecture/03_RISK_MITIGATION_AND_DISCLAIMER_FRAMEWORK.md) |
...
| **C — SliverVine ExoMesh** | **Sub-ms automated shield** + pre-broadcast severance | Stays **100% dry** at the station | **`lostUsd ≡ 0`** · soil fuse **p50 ~106µs** `checkSoilResistance()` |
...
| **0:24–0:30** | Option C umbrella deploys as a **ReflexCore (SSRC) hex reflex** before the train doors | *"Option C: SliverVine ExoMesh — sub-ms shield, then the train. Never the other way around."* | Identity sentence on-screen |
