/**
 * Worker cold-tier SSOT — `sanctuary_invariants.wasm` host export (`citadel_evaluate_packed`).
 */
...
type SanctuaryWasmExports = {
  ...
  citadel_evaluate_packed: (inPtr: number, outPtr: number) => number;
};
...
function bindSanctuaryWasm(bytes: Uint8Array): boolean {
  try {
    ...
    if (typeof ex.citadel_evaluate_packed !== "function") return false;
    exportsRef = ex;
    wasmU8 = null;
    return true;
  } catch {
...
export function evaluateSanctuaryPacked(packed: Uint8Array): SanctuaryEvalResult | null {
  if (!ensureSanctuaryWasmRuntime() || !exportsRef) return null;
  const mem = bindViews();
  if (!mem) return null;
  mem.set(packed.subarray(0, SANCTUARY_PACKED_LEN), IN_OFFSET);
  const rc = exportsRef.citadel_evaluate_packed(IN_OFFSET, OUT_OFFSET);
  if (rc !== 0) return null;
