...
> **SSOT Lock:** **218 test files | 1032 PASS clean (100%)** · **Release: v0.95 Santenmoku Core** · **3-Tier Security Scorecard: 5/0/0 PASS** · Gate `0xb174118bC0B84e8D6D59EEF2339e29bF7FCf8BF1` · Wasm **<28kb / <60µs** · ABI **v2** · 28-protocol-slot FFI (RESERVED_ABI_V2 holes preserved)  
...
| **On threat** | Out-of-scope calldata (cross-chain drift) | **p50 ~15µs** — severs EIP-712 channel · **$0 Gas** |
...
**Industry's first EIP-1193 Edge-Wasm 0-Gas Pre-Consensus Reference Implementation** for ERC-8196 · ERC-8226 · EIP-8079.
...
| **Vitest** | **218 test files \| 1032 PASS clean** · `pnpm test -- --run` |
| **C-End SDK** | `@slivervine/robinhood-agentic-retail-wallet-guard` · EIP-1193 · **35/35** retail guard tests |
...
SliverVine is a **pre-consensus execution safety primitive** — E2E Edge Shield **`checkSoilResistance()`** (**p50 ~106µs**) + reflex-core **`rootProtection()`** (**p50 ~15µs** on `--trip`) + immutable **EIP-712 consume-once `SliverVineGate`** on Arbitrum One.

**Production highlights:** GMX v2 · Pendle Institutional Sentinel · **5-Core Venue Matrix** · **EIP-1193 Retail Guard SDK** · Stabilizer Sepolia sandbox · **v1.0 public gateway** (`X-Citadel-Tier: public` · 5 RPS).
...
|---|----------|-------------------|
| **1** | **EIP-1193** | `npx vitest run tests/sdk/retail-guard-provider.test.ts` — `withRetailGuardProvider()` |
| **2** | **EIP-6963** | Same suite — `announceGuardedProvider()` announce/request |
| **3** | **EIP-712** | `pnpm demo:gmx -- --trip` — pre-sign severance · Gate `0xb174…` consume-once |
| **4** | **ERC-4337 / ERC-7579** | PolicyGuardV2 [`0xfd98cadb…8781`](https://arbiscan.io/address/0xfd98cadb7018f692ec58cd4359e0c0399f4f8781) |
| **5** | **ERC-2612 / Permit2** | Retail guard Vitest — Permit2 approve/permit block paths |

Wiki SSOT → [`docs/architecture/04_STANDARD_COMPLIANCE_AND_EIP_WIKI.md`](./docs/architecture/04_STANDARD_COMPLIANCE_AND_EIP_WIKI.md) · ERC-8196 = factual EIP attribution only (not a venue adapter)
...
```bash
pnpm test -- --run          # 218 test files | 1032 PASS clean
...
| Tier | Commands | Scope |
|------|----------|-------|
| **Tier 0 — Retail Guard** | `npx vitest run tests/sdk/retail-guard-provider.test.ts` | EIP-1193 C-end middleware |
...
| **Composable primitive** | `@slivervine/citadel-sdk` · `withCitadelShield` · Retail Guard EIP-1193 |
