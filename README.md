...
> **SSOT lock (Buildathon):** v1.0 Delivered (Sepolia + Arbitrum One verified) · Vitest **181 test files | 806 PASS Clean** · deep fuzz **327,675** via `pnpm audit:nightly` · Tier-0 Docker [`Dockerfile`](./Dockerfile) · docs hub [`docs/README.md`](./docs/README.md) · **Judge quick brief:** [`JUDGE_BRIEF.md`](./JUDGE_BRIEF.md)
...
Wayfinder: `pnpm demo:wayfinder` (Normal Route Interception) · `pnpm demo:wayfinder -- --trip` (0-Gas Fail-Closed Soil Trip)
...
Zero-dependency container execution — no host Node/pnpm install. Runs the isolated 5-step `demo:e2e` dry-run and Tier-1 ANSI HUD demo. Full regression: `docker run --rm slivervine-citadel pnpm test` (**181 test files | 806 PASS Clean**). Sidecar express audit → [`docker/README.md`](./docker/README.md).
...
2. `pnpm test` verifies **181 test files | 806 PASS Clean**.
...
* **Vitest SSOT**: 🧪 **181 test files | 806 PASS Clean** on `pnpm test -- --run`
...
| **M2: Pre-Execution Radar** | ✅ Delivered | `checkSoilResistance()` engine, **181 test files | 806 PASS Clean**, 91.2 KiB gzip bundle, sub-ms latency. |
