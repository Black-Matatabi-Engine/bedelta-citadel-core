...
import { E2E_LOST_USD_INVARIANT, E2E_SUMMARY_DIVIDER } from "./e2e-demo-constants";
...
export function printE2eSummaryHud(
  ...
  e2eLog(
    `• Capital Split Routing:    GMX GM Pool ${fmtE2eUsd(ledger.gmxDepositUsd)} ║ HL L1 Margin ${fmtE2eUsd(ledger.hlMarginUsd)}`,
  );
  e2eLog(
    `• Delta Neutral Exposure:    GMX Long +${fmtE2eUsd(ledger.gmxEffectiveLongUsd)} ║ HL Short -${fmtE2eUsd(ledger.hlHedgeShortUsd)}`,
  );
  e2eLog(`• Protocol Treasury Revenue: +${fmtE2eUsd(ledger.builderRebateEarnedUsd)} USD (uiFeeReceiver GMX Share)`);
  e2eLog(`• Final User Principal:     ${fmtE2eUsd(ledger.finalUserVaultBalanceUsd)} ${ledger.token} (100% Principal Guarded)`);
  e2eLog(`• Invariant Verification:   ${E2E_LOST_USD_INVARIANT} · Δnet ≡ ${ledger.deltaNetEthFormatted} ETH  [ VERIFIED ]`);
  e2eLog("");
