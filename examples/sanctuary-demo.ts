...
wrapDemoExecution(async (ctx) => {
  ...
  if (!jsonMode) {
    ...
    if (interactive) printSanctuaryProofHighlights();
  }

  const results: SanctuaryScenarioJsonResult[] = [];
  results.push(runScenarioA({ interactive }));
  if (interactive) await awaitSanctuaryScenarioTransition("B");
  results.push(runScenarioB({ interactive }));
  if (interactive) await awaitSanctuaryScenarioTransition("C");
  results.push(runScenarioC({ interactive }));

  const timestamp = new Date(ctx.nowMs).toISOString();
  const payload = buildSanctuaryRunPayload(results, benchmark, timestamp);
