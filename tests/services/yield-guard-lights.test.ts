...
function baseResult(overrides: Partial<YieldRouterResult> = {}): YieldRouterResult {
  return {
    ...
    venues: [
      ...
      {
        venue: "jupiter",
        depth: { venue: "jupiter", symbol: "ETH", depthUsd: 1e6, spotPrice: 3000, perpPrice: 3000, fetchedAt: "" },
        apy: 0.08,
        edgeBps: 0,
        health: { ok: true, latencyMs: 10, reasons: [] },
      },
...
describe("buildAdaptiveGuardLights", () => {
  it("returns green when soil and venues healthy", () => {
    ...
    expect(lights).toEqual({ hyperliquid: "green", jupiter: "green", polymarket: "green" });
  });

  it("marks jupiter red on slippage trip", () => {
    ...
    expect(lights.jupiter).toBe("red");
    expect(lights.polymarket).toBe("red");
  });
});
