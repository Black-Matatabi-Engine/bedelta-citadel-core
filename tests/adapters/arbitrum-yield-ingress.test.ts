...
describe("multi-chain ingress router", () => {
  it("parseIngressChain accepts SOLANA and ARBITRUM", () => {
    expect(parseIngressChain("solana")).toBe("SOLANA");
    expect(parseIngressChain("ARBITRUM")).toBe("ARBITRUM");
    expect(parseIngressChain(null)).toBe("SOLANA");
  });

  it("resolveYieldStack uses Arbitrum stable yields when ingressChain=ARBITRUM", async () => {
